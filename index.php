...
<div class="flex min-h-screen">
  <div class="w-64 bg-gradient-to-b from-green-700 to-green-900 text-white p-5 shadow-lg">
    ...
    <nav class="space-y-3">
      <a href="index.php?page=dashboard" class="block px-4 py-2 rounded transition <?= $page=='dashboard' ? 'bg-green-600' : 'hover:bg-green-500' ?>">🏠 Dashboard</a>
      <a href="index.php?page=request" class="block px-4 py-2 rounded transition <?= $page=='request' ? 'bg-green-600' : 'hover:bg-green-500' ?>">📋 Ticket</a>
      <a href="index.php?page=records" class="block px-4 py-2 rounded transition <?= $page=='records' ? 'bg-green-600' : 'hover:bg-green-500' ?>">📁 Records</a>
    </nav>
  </div>
  ...
  <div class="flex-1 p-6">
    <div class="flex justify-end items-center gap-4 mb-8">
        <div class="flex items-center gap-2 bg-white px-4 py-2 rounded-lg shadow-sm border">
            ...
            <span class="text-blue-700 font-bold"><?php echo htmlspecialchars($user_data['user_name']); ?></span>
        </div>
